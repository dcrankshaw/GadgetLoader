/*Dont delete all, someone can be deleted*/
...
{
    /// <summary>
    /// A struct holding all of the data associated with a particle
    /// simulation particle at a particular point in time.
    /// </summary>
...
//            writer.Write(veldisp);
        ...
        /// <summary>
        /// Writes the positions, velocites, and IDs of particles to different files so they can
        /// be written to seperate tables (and possibly indexed differently).
        /// This allows for database denormalization.
        /// </summary>
        /// <param name="idwriter"></param>
        /// <param name="poswriter"></param>
        /// <param name="velwriter"></param>
        public void WriteBinarySplitTables(BinaryWriter idwriter, BinaryWriter poswriter, BinaryWriter velwriter)
        {
            idwriter.Write(snapnum);
            idwriter.Write(phkey);
            idwriter.Write(id);
        ...
        public void WritePos(BinaryWriter poswriter)
        {
            poswriter.Write(snapnum);
            poswriter.Write(id);
            poswriter.Write(x);
            poswriter.Write(y);
            poswriter.Write(z);
        ...
        public void WriteVel(BinaryWriter velwriter)
        {
            velwriter.Write(snapnum);
            velwriter.Write(id);
            velwriter.Write(vx);
            velwriter.Write(vy);
            velwriter.Write(vz);
    ...
    /// <summary>
    /// Particle data from file where particles ordered according to FOF/SubHalo hierarchy.
    /// </summary>
...
//          public float  StB       ;//           : 0.0, $
    ...
    public class ParticleComparatorByID : IComparer<Structs>
    {
        public int Compare(Structs one, Structs two)
        {
            int comp = one.snapnum.CompareTo(two.snapnum);
