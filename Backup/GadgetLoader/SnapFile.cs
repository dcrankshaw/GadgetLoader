...
namespace GadgetLoader
{
    ...
    class SnapFile
    {
        ...
        public SnapFile(String filename, float samplingRate)
        {
            bool hasSampling = true;
            ...
            using (BinaryReader reader = new BinaryReader(new FileStream(filename, FileMode.Open, FileAccess.Read)))
            {
                ...
                if (hasSampling)
                {
                    ...
                    sampling = new bool[np];
                    if (hasSampling)
                    {
                        for (int i = 0; i < np; i++)
                        {
                            sampling[i] = (rand.NextDouble() <= samplingRate);
                            if (sampling[i])
                                numSample++;
                        }
                    }
                    DebugOut.PrintLine("Sampling Snapshot leaves "+numSample+ " out of "+np);
                }
                else
                    numSample = np;
                ...
                if (hasSampling)
                    ReadDataSampled(reader, sampling);
                else
                    ReadData(reader);
            }
        }

        private void ReadDataSampled(BinaryReader reader, bool[] sampling)
        {
            int bytesRead = 0;
            int record = reader.ReadInt32();
            int np = sampling.Length;
            int count = 0;
            for (int i = 0; i < np; i++)
            {
                if (sampling[i])
                {
                    pos[count, 0] = reader.ReadSingle();
                    pos[count, 1] = reader.ReadSingle();
                    pos[count++, 2] = reader.ReadSingle();
                }
                else
                    reader.ReadBytes(12);
                bytesRead += 12;
            ...
            for (int i = 0; i < np; i++)
            {
                if (sampling[i])
                {
                    vel[count, 0] = reader.ReadSingle();
                    vel[count, 1] = reader.ReadSingle();
                    vel[count++, 2] = reader.ReadSingle();
                }
                else
                    reader.ReadBytes(12);
                bytesRead += 12;
