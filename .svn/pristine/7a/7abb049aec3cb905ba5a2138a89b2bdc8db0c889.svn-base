    ...
    public class Filter<T>
    {
        ...
        public Filter(byte[] existingFilter, HashFunction hashFunction, int k)
        {
            if (hashFunction == null)
            {
                ...
                {
                    throw new ArgumentNullException("hashFunction", "Please provide a hash function for your type T, when T is not a string, int, or long.");
                }
            }
            else
        ...
        public List<int> getSetPositions()
        {
        ...
        public int intersectWith(Filter<T> other)
        {
            if (other.getTotalSize() == this.getTotalSize())
            {
                BitArray otherFilter = other.convertToBitArray();
                int count = 0;
                for (int i = 0; i < hashBits.Count; i++)
                {
                    if (otherFilter[i] && hashBits[i])
                    {
                        count++;
                    }
                }
